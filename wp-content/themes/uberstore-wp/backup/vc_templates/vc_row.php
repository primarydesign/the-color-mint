...
if ( $type !== 'in_container' ) {
	$output .= '<div id="'.$row_id.'" class="'.$type_class.' '.$parallax.$video.$el_class.vc_shortcode_custom_css_class($css, ' ').'" '.$data.'>';
}

$output .= $bg_video;
$output .= '<div '.(( $type == 'in_container' ) ? ' id="'.$row_id.'"' : '').' class="'. (( $type !== 'in_container' ) ? 'row' : $css_class.$parallax.$el_class.vc_shortcode_custom_css_class($css, ' ')) .'"'. (( $type != 'in_container' ) ? '' : $data) .'>';
